...
TRAIN_SET, TEST_SET = \
    torch.utils.data.Subset(TRAIN_SET, range(0, 5000)), \
...
def BatchThisModel(theModel:A4Model,
    ...
    """
        Batch this model for one epoch, give me the model optimizer and some extra thing,
        then it will collect the average loss of one epoch.
        Note:
        This one is for Regression Model.
    :param theModel:
    :param dataLoader:
    :param transform:
    :return:
    ...
    for II, (X, _) in enumerate(tqdm(dataLoader)):
        ...
        if optimizer is None:
            with torch.no_grad():
                AvgLoss += float(theModel.FeedForward(X))/X.shape[0]
        else:
            optimizer.zero_grad()
            Loss = theModel.FeedForward(X)
            AvgLoss += Loss.item() / X.shape[0]
            Loss.backward()
            optimizer.step()
    with torch.no_grad():
        Loss = theModel.FeedForward(X)
...
def main():
    def A4Run(NonLin = False):
        Bs, Epochs, h, lr = 100, 30, 64, 0.01
        Losses = []
